...
На этом этапе камера не запускает измерения и не привязывает файлы к пикселям. Сервис Raspberry Pi, пример `systemd` для пользователя `user` и инструкция установки находятся в `raspberry_camera_service/`.
