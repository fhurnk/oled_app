...
В `v1.8.0-alpha` автоматический поиск спектрометра изолирован от GUI тайм-аутом: зависший SeaBreeze не блокирует запуск приложения. При подборе времени интегрирования широкая FWHM больше не считается ошибкой `TOO_WIDE`, а статус определяется по raw-сигналу до вычитания фона. Найденное время интегрирования переносится на следующую точку для экспозиций выше и ниже 10 мс и автоматически уменьшается после raw-пика выше цели. В настройках времена спектрометра задаются в миллисекундах.
