...
- спектральную светимость как `скорректированный интеграл × интегральный коэффициент × поправка четверти × геометрический коэффициент`.

Цвет задается для серии и при необходимости отдельно для четвертей. При измерениях приложение выбирает коэффициент яркости по цвету четверти. Калибровка одного пикселя задаёт безразмерную поправку для всей его физической четверти; до калибровки эта поправка равна `1`.

Кнопка пакетного пересчёта применяет текущие коэффициенты к существующим XLSX серии. Если соответствующего raw CSV нет, данные восстанавливаются из книги перед её атомарной заменой.
...
Спектральная чувствительность не пересчитывается автоматически после съёмки. Отдельная кнопка читает готовый `Processed counts per s`, нормирует каждую кривую на максимум, применяет `CIE V(λ) / BPW34(λ)` и интегрирует. Перед аппроксимацией светимость фотодиода рассчитывается с действующим коэффициентом R/G/B четверти и геометрическим коэффициентом. Абсолютный интеграл сопоставляется с этой светимостью на нескольких напряжениях. Сохранённый коэффициент можно применить к другим пикселям четверти или заменить новой калибровкой. Исходный XLSX остаётся неизменным, а все результаты и коэффициенты сохраняются в отдельном `SPECTRAL_RECALC_*.xlsx`.
