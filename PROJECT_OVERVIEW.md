...
7. Составить Origin-отчет по серии через кнопку `Составить отчет`: для каждой подсерии выбрать одну снятую подложку, затем её спектральный пиксель и сетку напряжений.
...
Окно спектров поддерживает одиночный пиксель и режим `Вся подложка последовательно`. Во втором режиме выбирается одна подложка, её доступные пиксели проходят по очереди, а старт от напряжения открытия использует индивидуальное значение журнала для каждого пикселя.
