...
- для стабильности пользователь выбирает, создавать ли отдельный `*_telemetry.mp4`; при объединении локальный FFmpeg добавляет сверху полосу с временем, измеренными U/I и отметкой лимита тока, сохраняя исходный кадр и исходный MP4 без изменений.
...
7. Составить Origin-отчет по серии через кнопку `Составить отчет`: выбрать полный состав, только ВАЯХ или только спектры, при необходимости исключить четверти `1–4`; при наличии спектров для каждой оставшейся подсерии выбрать одну снятую подложку, затем её спектральный пиксель и сетку напряжений.
