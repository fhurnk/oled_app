...
The current prerelease line is `v1.8.0-beta`. It adds immediate voltage-mode stability targets with symmetric setpoint buttons, numbered camera-only measurement sessions with captured-photo confirmation, live integration-time trial spectra, persistent manual spectrum start voltage, and safe spectrum stopping; publish it as a GitHub prerelease.
...
- Stability: modular workflow is in `oled_app/measurements/stability.py` and the modular GUI window is in `oled_app/gui/stability_window.py`; beta supports mutable constant-current feedback and immediate constant-voltage targets and records both target and applied values in raw CSV/XLSX.
